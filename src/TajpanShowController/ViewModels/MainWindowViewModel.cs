...
public sealed partial class MainWindowViewModel : ObservableObject, IAsyncDisposable
{
    ...
    [ObservableProperty] private string connectionDetail = "—   115200 baud";
    ...
    private void SetPlaylistModified(bool modified) => IsPlaylistModified = modified;
    private void RaiseTrackProperties() { OnPropertyChanged(nameof(DisplayedTrack)); OnPropertyChanged(nameof(NowPlayingFilename)); OnPropertyChanged(nameof(LcdLine1)); OnPropertyChanged(nameof(IsSeekEnabled)); }
    ...
    public async ValueTask DisposeAsync() { _playingBlink.Stop(); _debugUiTimer.Stop(); await _connectionCoordinator.DisposeAsync(); await SaveAsync(); await _remote.DisposeAsync(); await _playback.DisposeAsync(); _saveGate.Dispose(); }
}
