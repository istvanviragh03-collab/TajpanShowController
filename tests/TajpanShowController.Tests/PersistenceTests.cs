...
public sealed class PersistenceTests : IDisposable
{
    ...
    {
        var store = new JsonSettingsStore(_directory); var expected = new AppSettings { PlaylistName = "Saved show", Volume = .5f, AudioOutputDeviceNumber = 2, AudioOutputDeviceName = "Test output", Playlist = [new PlaylistTrack { FilePath = "x.wav", Title = "X", Duration = TimeSpan.FromSeconds(2) }] };
        var ct = TestContext.Current.CancellationToken; await store.SaveAsync(expected, ct); var loaded = await store.LoadAsync(ct); Assert.Single(loaded.Playlist); Assert.Equal("Saved show", loaded.PlaylistName); Assert.Equal("X", loaded.Playlist[0].Title); Assert.Equal(.5f, loaded.Volume); Assert.Equal(2, loaded.AudioOutputDeviceNumber); Assert.Equal("Test output", loaded.AudioOutputDeviceName);
    }
